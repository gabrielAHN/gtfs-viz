...
function StopsMapPage() {
  ...
  useEffect(() => {
    if (search.selectedStopId && allStops && Array.isArray(allStops)) {
      // Use loose comparison (==) to handle string vs number stop_id from DuckDB
      const stop = allStops.find((s: any) => String(s.stop_id) === String(search.selectedStopId));

      const currentStopId = ClickInfo?.object?.stop_id || ClickInfo?.stop_id;

      if (stop && String(currentStopId) !== String(search.selectedStopId)) {
        setClickInfo(stop);

        const lat = Number(stop.stop_lat);
        const lon = Number(stop.stop_lon);
        if (!search.mapFocus && Number.isFinite(lat) && Number.isFinite(lon)) {
      ...
      <div className="flex flex-col gap-4">
        ...
        {isLoading ? (
          ...
          <div className="relative h-[74vh] w-full border p-1 rounded-md overflow-hidden flex items-center justify-center">
            <div className="text-sm text-muted-foreground">No stops data available.</div>
          </div>
        ) : (
          <>
            <StopStationForm
            ...
            <div className="relative h-[74vh] w-full overflow-hidden">
              <StopsMap
                ...
                initialStopId={search.selectedStopId}
              />
            </div>
          </>
        )}
