...
const RoutesTable = ({ FileTypes, setFileTypes }: any) => {
  ...
  const [originalShapeMap, setOriginalShapeMap] = useState<
    ...
    return (
      <>
        ...
        {hasOriginal && hasShapeComparison && (
          <Button variant="outline" onClick={() => setShowCompare((prev) => !prev)}>
  ...
  const renderSelectedSupplementaryRows = ({ row, originalRow, columns }: any) => {
        ...
        <TableCell colSpan={columns.length} className="p-3">
          <div className="grid grid-cols-2 gap-4">
            <ShapeMiniMap
              ...
              conn={conn}
            />
            <ShapeMiniMap points={editedPoints} color={routeColor} label="Edited" conn={conn} />
          </div>
        </TableCell>
      </TableRow>
    )
          ...
          cell: ({ row }: any) => {
            ...
            return (
              <span className="flex items-center gap-2">
                <span className="h-3 w-6 rounded-sm border" style={{ backgroundColor: hex }} />
          ...
          cell: ({ row }: any) => {
            ...
            switch (shapeStatus) {
              case "modified":
                return <span className="text-yellow-600 dark:text-yellow-400">Modified</span>
              case "added":
                return <span className="text-green-600 dark:text-green-400">Added</span>
              case "removed":
                return <span className="text-red-600 dark:text-red-400">Removed</span>
              case "new":
