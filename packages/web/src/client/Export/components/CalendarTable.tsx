...
const CalendarTable = ({ FileTypes, setFileTypes }: { FileTypes: any; setFileTypes: any }) => {
  ...
  const { data: serviceRouteMap = new Map<string, string>() } = useQuery({
    ...
    queryFn: async () => {
      ...
      const newCals = await executeQuery(
        table.conn,
        "SELECT service_id FROM CalendarTable WHERE status IN ('new', 'new edit')",
      )
      if (newCals.length > 0) {
        const firstRoute = await executeQuery(
          table.conn,
          "SELECT DISTINCT route_id FROM TripsTable WHERE route_id IS NOT NULL LIMIT 1",
        )
        if (firstRoute.length > 0) {
          const rid = String(firstRoute[0].route_id)
          for (const c of newCals) {
            if (!m.has(String(c.service_id))) m.set(String(c.service_id), rid)
          }
        }
      }
      return m
      ...
      renderSelectionActions={({ clickInfo: ci }: any) => {
        ...
        return rid ? (
          <Button asChild variant="secondary" size="sm">
            <Link
              to="/routes/service"
              search={{ selectedRouteId: rid, selectedServiceId: String(ci.service_id) }}
            >
