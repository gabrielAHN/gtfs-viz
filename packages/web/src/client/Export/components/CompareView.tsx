...
export function diffStopLists(orig: any[], edited: any[]) {
  ...
  for (let i = 1; i <= m; i++)
    for (let j = 1; j <= n; j++)
      dp[i][j] =
        oIds[i - 1] === eIds[j - 1] ? dp[i - 1][j - 1] + 1 : Math.max(dp[i - 1][j], dp[i][j - 1])
  ...
  while (i > 0 || j > 0) {
    if (i > 0 && j > 0 && oIds[i - 1] === eIds[j - 1]) {
      const o = orig[i - 1],
        e = edited[j - 1]
      const changed =
        o.arrival_time !== e.arrival_time ||
        o.departure_time !== e.departure_time ||
        o.stop_sequence !== e.stop_sequence
      ...
      <div className="overflow-auto max-h-[50vh]">
        <table className="w-full text-xs border-collapse">
          <thead className="bg-muted sticky top-0 z-10">
            <tr>
              {/* Original side */}
              <th className="px-2 py-1.5 text-left font-semibold text-red-700 dark:text-red-400 border-r w-6"></th>
              {fields.map((f) => (
                <th
                  key={`o-${f.key}`}
                  className="px-2 py-1.5 text-left font-medium border-r last:border-r-2 last:border-r-border"
                >
                  {f.label}
                </th>
              ))}
              {/* Edited side */}
              <th className="px-2 py-1.5 text-left font-semibold text-green-700 dark:text-green-400 w-6"></th>
              {fields.map((f) => (
            ...
            <tr>
              <th
                ...
                className="px-2 py-0.5 text-[10px] font-semibold text-red-600 dark:text-red-400 bg-red-50/50 dark:bg-red-950/20 border-r-2 border-r-border text-center"
              >
                Original
              </th>
              <th
                colSpan={fields.length + 1}
                className="px-2 py-0.5 text-[10px] font-semibold text-green-600 dark:text-green-400 bg-green-50/50 dark:bg-green-950/20 text-center"
              >
                Edited
              </th>
            </tr>
          ...
          <tbody>
            {diffRows.map((d, i) => {
                ...
                return (
                  <tr key={i} className="border-t">
                    ...
                    {fields.map((f) => (
                      <td
                        key={f.key}
                        className="px-2 py-1 bg-red-50 dark:bg-red-950/20 text-red-600 line-through border-r last:border-r-2 last:border-r-border"
                      >
                ...
                return (
                  <tr key={i} className="border-t">
                    ...
                    {fields.map((f) => (
                      <td
                        key={f.key}
                        className="px-2 py-1 bg-muted/30 border-r last:border-r-2 last:border-r-border"
                      />
                    ))}
                    ...
                    {fields.map((f) => (
                      <td
                        key={f.key}
                        className="px-2 py-1 bg-green-50 dark:bg-green-950/20 text-green-600 font-medium"
                      >
              ...
              return (
                <tr
                  key={i}
                  className={`border-t ${isChanged ? "bg-amber-50/30 dark:bg-amber-950/10" : ""}`}
                >
                  ...
                  {fields.map((f) => (
                    <td
                      key={f.key}
                      className={`px-2 py-1 border-r last:border-r-2 last:border-r-border ${
                        isChanged
                          ? cellClass(d.orig?.[f.key], d.edited?.[f.key]) === "font-medium"
