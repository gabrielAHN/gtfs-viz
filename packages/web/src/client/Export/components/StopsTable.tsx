...
const StopsTable = ({ FileTypes, setFileTypes }) => {
  ...
  const renderSelectedSupplementaryRows = ({
    ...
    const isDowngrade =
      originalRow.location_type_name === "Station" && row.location_type_name === "Stop"
            ...
            <TableCell colSpan={columns.length} className="p-3">
              <div className="flex items-center gap-2">
                <span className="text-sm font-medium text-purple-800 dark:text-purple-300">
                  🔗 Parent station was detached because station "{originalRow.parent_station}" was
          ...
          cell: ({ row }) => {
            ...
            if ((status === "edit" || status === "new edit") && originalData) {
              if (
                ...
                currentStop.location_type_name === "Stop"
              ) {
                ...
                if (droppedParts.length > 0) {
                  return (
                    <span className="text-orange-600 dark:text-orange-400 font-medium">
                      🔻 Downgraded from Station ({droppedParts.length} part
                      {droppedParts.length > 1 ? "s" : ""} dropped)
                    </span>
                  )
                }
                return (
              ...
              if (
                ...
                (!currentStop.parent_station || currentStop.parent_station === "")
              ) {
                const parentInTable = tableData.find(
                  (item: any) => item.stop_id === originalData.parent_station,
                )
