    ...
    () => [
      { accessorKey: "trip_id", header: "Trip ID" },
      {
        ...
        cell: ({ row }) => {
          ...
          return (
            <Link
              to="/routes/info"
              search={{ selectedRouteId: t.route_id }}
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          return (
            <Link
              to={hasStopTimes ? "/routes/service" : "/routes/info"}
              search={{
                selectedRouteId: t.route_id,
                ...(hasStopTimes ? { selectedServiceId: t.service_id } : {}),
              }}
              onClick={(e) => e.stopPropagation()}
      ...
      ...(hasStopTimes
        ? [
            {
              accessorKey: "first_departure_seconds",
              header: "Departure",
              cell: ({ row }: any) => formatTripTime(row.original.first_departure_seconds) || "",
            },
            {
              accessorKey: "last_arrival_seconds",
              header: "Arrival",
              cell: ({ row }: any) => formatTripTime(row.original.last_arrival_seconds) || "",
            },
          ]
        : []),
