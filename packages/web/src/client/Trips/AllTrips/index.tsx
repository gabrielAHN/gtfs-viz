...
import {
  ...
  saveStopTimesEdits,
  fetchEditedTripStatuses,
  fetchStopsWithRouteFlag,
  fetchTripShapeBands,
...
function AllTrips({ allTrips, tripTimeBounds, hasStopTimes, search, updateSearch }: AllTripsProps) {
  ...
  const setCompareTripIds = useCallback(
  ...
  const [undoStack, setUndoStack] = useState<
    Array<{ stops: EditableStop[]; deleted: Set<number> }>
  >([])
  ...
  const pushEdit = useCallback((newStops: EditableStop[], newDeleted?: Set<number>) => {
    setUndoStack((prev) => [
      ...prev.slice(-19),
      { stops: editStopsRef.current, deleted: new Set(deletedStopIndicesRef.current) },
    ])
  ...
  const availableTripIds = useMemo(
    () =>
      tripsForTripIdFilter
        .map((t) => ({ value: String(t.trip_id), label: String(t.trip_id) }))
  ...
  const availableRouteIds = useMemo(() => {
    ...
    return tripsForRouteFilter
      .filter((t) => {
        if (!t.route_id || seen.has(t.route_id)) return false
        seen.add(t.route_id)
        return true
      })
      .map((t) => ({
  ...
  const availableRouteTypes = useMemo(() => {
    ...
    for (const t of tripsForTypeFilter) {
      if (t.route_type_name) types.add(t.route_type_name)
    }
    return Array.from(types)
      .sort()
      .map((type) => ({ label: type, value: type, color: getRouteTypeColor(type) }))
  }, [tripsForTypeFilter])
  ...
  const {
    ...
    enabled: !!conn && !!initialized && !!selectedTripId && hasStopTimes && stopTimes.length > 1,
    staleTime: REROUTE_QUERY_STALE_TIME,
  })

  const compareTrips = useMemo(
    () =>
      compareTripIds.flatMap((id) => {
  ...
  const { data: compareStopTimesMap = {}, isLoading: compareLoading } = useQuery({
    ...
    queryFn: async () => {
      const entries = await Promise.all(
        compareTripIds.map(
          async (id) => [id, await fetchServiceTripStopTimesData(conn, id)] as const,
        ),
      )
  ...
  const compareLineTripIds = useMemo(() => {
    const ids = [selectedTripId, ...compareTripIds].filter(Boolean) as string[]
    return Array.from(new Set(ids))
  }, [selectedTripId, compareTripIds])

  const { data: compareOffsetPaths = {} } = useQuery({
    queryKey: ["fetchTripShapeBands", compareLineTripIds],
    queryFn: async () => fetchTripShapeBands(conn, compareLineTripIds),
    enabled: !!conn && !!initialized && tripView === "map" && compareLineTripIds.length > 1,
    retry: false,
  })
  ...
  const handleDeleteStop = useCallback(
    (stopIdx: number) => {
      setUndoStack((prev) => [
        ...prev.slice(-19),
        { stops: editStops, deleted: new Set(deletedStopIndices) },
      ])
  ...
  const handleReplaceStop = useCallback(
    (
      stopIdx: number,
      newStop: { stop_id: string; stop_name: string; stop_lat?: number; stop_lon?: number },
    ) => {
      setEditStops((prev) =>
        prev.map((s, i) =>
          i === stopIdx
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: (updatedStopTimes) => {
      setIsEditing(false)
      setEditErrors([])
      queryClient.setQueryData(["fetchServiceTripStopTimesData", selectedTripId], updatedStopTimes)
  ...
  const handleEditStopUpdate = (stops: EditableStop[]) => {
    pushEdit(stops)
  }
  const handleTimelineStopUpdate = (
    _ti: number,
    si: number,
  ...
  const selectedAddStop = addStopId
    ? routeStops.find((rs: any) => rs.stop_id === addStopId)
    : undefined
  ...
  const handleAddStopConfirm = () => {
    ...
    const newStop = {
      ...
      trip_id: "",
      stop_sequence: 0,
      stop_id: selectedAddStop.stop_id,
      stop_name: selectedAddStop.stop_name,
  ...
  const renderStopPanel = (compact?: boolean) => (
    <StopEditPanel
      ...
      noStops={editStops.length === 0}
      addStopId={addStopId}
      addArrival={addArrival}
      addDeparture={addDeparture}
      addStopSequence={addStopSequence}
      addStopSeqManual={addStopSeqManual}
      addStopOptions={addStopOptions}
      selectedAddStop={selectedAddStop}
      onPushEdit={pushEdit}
      onSetSelectedEditIdx={setSelectedEditIdx}
      onSetAddStopId={setAddStopId}
      onSetAddArrival={setAddArrival}
      onSetAddDeparture={setAddDeparture}
      onSetAddStopSeqManual={setAddStopSeqManual}
      onHandleReorderStop={handleReorderStop}
      onHandleAddStopConfirm={handleAddStopConfirm}
      onScrollToStop={scrollToStop}
      editStopsRef={editStopsRef}
        ...
        {tripsList.map((t, idx) => {
          ...
          return (
            ...
            >
              <span
                ...
                style={{
                  ...
                  backgroundColor: isHidden
                    ? "transparent"
                    : TRIP_LINE_COLORS[ci % TRIP_LINE_COLORS.length],
                  borderColor: TRIP_LINE_COLORS[ci % TRIP_LINE_COLORS.length],
                }}
              />
              <span
                className={`truncate max-w-[120px] font-medium ${isHidden ? "line-through opacity-50" : ""}`}
              >
                {t.trip_headsign || t.trip_id}
              </span>
              {t.route_name && (
                <span
                  className={`text-muted-foreground truncate max-w-[80px] ${isHidden ? "opacity-50" : ""}`}
                >
                  {t.route_name}
                </span>
              )}
              {t.service_id && (
                <span
                  className={`text-muted-foreground/60 truncate max-w-[60px] ${isHidden ? "opacity-50" : ""}`}
                >
                  {t.service_id}
                </span>
              )}
    ...
    <div className="space-y-4">
      ...
      {!selectedTrip && !isEditing && !isPicking && (
        <TripsHeader
          ...
          routeId={routeId}
          routeType={routeType}
          availableTripIds={availableTripIds}
          availableRouteIds={availableRouteIds}
          availableRouteTypes={availableRouteTypes}
          timeRange={timeRange}
          sliderBounds={sliderBounds}
          tripsCount={tripsWithStatus.length}
          hasActiveFilters={hasActiveFilters}
          tableSortingCount={tableSorting.length}
          ...
          return (
            <div className="rounded-md border border-red-300 bg-red-50 dark:bg-red-950/30 p-3 text-sm text-red-700 dark:text-red-300">
              ...
              >
                ...
                <span>
                  {editErrors.length} validation error{editErrors.length !== 1 ? "s" : ""}
                </span>
              </button>
      ...
      {selectedTrip && !isEditing && (
        <div
          ...
          onClick={() => handleTripSelect(undefined)}
          role="button"
          tabIndex={0}
          onKeyDown={(e) => {
            if (e.key === "Enter" || e.key === " ") handleTripSelect(undefined)
          }}
        >
          ...
          <span
            ...
            style={{
              backgroundColor:
                selectedTrip.route_color_hex || getRouteTypeColor(selectedTrip.route_type_name),
            }}
          />
          <div className="flex flex-wrap items-center gap-x-4 gap-y-1 min-w-0">
            ...
            {hasStopTimes && (
              <span className="text-sm">
                {formatTripTime(selectedTrip.first_departure_seconds) || "?"} {"\u2192"}{" "}
                {formatTripTime(selectedTrip.last_arrival_seconds) || "?"}
              </span>
            )}
          </div>
        </div>
        ...
        <div className="flex min-w-0 items-center gap-3 rounded-md border border-primary/50 bg-primary/5 px-4 py-2.5">
          ...
          {(() => {
            ...
            for (let i = 0; i < editStops.length; i++) {
              ...
              if (
                !orig ||
                editStops[i].stop_id !== orig.stop_id ||
                editStops[i].arrival_time !== orig.arrival_time ||
                editStops[i].departure_time !== orig.departure_time
              )
                changed++
            }
            changed += deletedStopIndices.size
            if (editStops.length !== stopTimes.length)
              changed = Math.max(changed, Math.abs(editStops.length - stopTimes.length))
            return changed > 0 ? (
          ...
          {(() => {
            const noChanges =
              JSON.stringify(editStops) ===
                JSON.stringify(stopTimes.map((s, i) => ({ ...s, _idx: i }))) &&
              deletedStopIndices.size === 0
            ...
            return (
              <div className="ml-auto flex items-center gap-1.5 shrink-0">
                ...
                <Button
                  variant="ghost"
                  size="sm"
                  disabled={noChanges || saveMutation.isPending}
                  onClick={() => {
                    setEditStops(stopTimes.map((s, i) => ({ ...s, _idx: i })))
                    setDeletedStopIndices(new Set())
                    setEditErrors([])
                ...
                <Button
                  ...
                  disabled={saveMutation.isPending || noChanges || noStops}
                  size="sm"
                  className="flex items-center h-7 text-xs px-3"
                >
                  <BiSave className="mr-1 h-3 w-3" />
      ...
      {selectedTrip && !isEditing && !isPicking && (
        <div className="flex gap-2">
          ...
          {hasStopTimes && compareTrips.length === 0 && (
            ...
            >
              <BiTransferAlt className="mr-1 h-4 w-4" />
              Reroute
            </Button>
          )}
          {hasStopTimes && compareTrips.length === 0 && stopTimes.length > 0 && (
            <Button
              ...
              className="flex items-center"
            >
              <BiGitCompare className="mr-1 h-4 w-4" />
              Compare
            </Button>
          )}
        </div>
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            <Button
              ...
              onClick={startPicking}
              size="sm"
              disabled={compareTrips.length >= maxCompareSlots}
            ...
            <Button
              ...
              onClick={() => {
                setCompareTripIds([])
                if (selectedTrip) updateSearch({ selectedTripId: undefined })
              }}
              size="sm"
            ...
            {compareTrips.map((ct, idx) => {
              ...
              return (
                <div
                  ...
                  onClick={() =>
                    setCompareTripIds((prev) => prev.filter((id) => id !== ct.trip_id))
                  }
                  role="button"
                  tabIndex={0}
                  onKeyDown={(e) => {
                ...
                >
                  ...
                  <span
                    ...
                    style={{
                      width: 8,
                      height: 8,
                      backgroundColor: TRIP_LINE_COLORS[colorIdx % TRIP_LINE_COLORS.length],
                    }}
                  />
        ...
        <TripsHeader
          ...
          extraButtons={
            <Button
              ...
              size="default"
              className="w-full md:w-auto flex items-center justify-center bg-primary/10 border-primary/50"
            >
      ...
      {((!selectedTrip && compareTrips.length === 0) || isPicking) && (
        <TableComponent
          ...
          data={
            isPicking
              ? filteredTrips.filter((t) => !allSelectedIds.has(String(t.trip_id)))
              : filteredTrips
          }
          columns={columns}
          ClickInfo={undefined}
        ...
        >
          <div className="text-sm text-muted-foreground p-3 border rounded-md bg-muted/30">
            {isPicking
              ? "Select a trip to compare with"
              : hasStopTimes
                ? "Select a trip row to view its stops"
                : "Select a trip row to view details"}
          </div>
        </TableComponent>
      )}
      ...
      {selectedTrip && !isPicking && !hasStopTimes && !saveMutation.isPending && (
        <div className="text-sm text-yellow-800 dark:text-yellow-200 p-3 border border-yellow-300 rounded-md bg-yellow-50 dark:bg-yellow-900/20">
          stop_times.txt not imported — stop times, editing, comparison, and timeline views are
          disabled
        </div>
      )}
      ...
      {selectedTrip && !isPicking && hasStopTimes && !saveMutation.isPending && (
        <div className="space-y-3">
          {!isEditing && (
            <div className="flex flex-wrap items-center gap-x-6 gap-y-2 rounded-md border p-3 text-sm">
              <div>
                ...
                <div
                  className={`font-medium ${!selectedTrip.first_departure_seconds && !selectedTrip.last_arrival_seconds ? "text-muted-foreground/50" : ""}`}
                >
                  {formatTripTime(selectedTrip.first_departure_seconds) || "\u2014"} {"\u2192"}{" "}
                  {formatTripTime(selectedTrip.last_arrival_seconds) || "\u2014"}
                </div>
              ...
              <div>
                <span className="text-xs text-muted-foreground">Headsign</span>
                <div
                  className={`font-medium ${!selectedTrip.trip_headsign ? "text-muted-foreground/50" : ""}`}
                >
              ...
              {selectedTrip.route_name && (
                <div>
                  ...
                  <div className="font-medium">
                    <Link
                      to="/routes/info"
                      search={{ selectedRouteId: selectedTrip.route_id }}
                ...
                <div>
                  ...
                  <div className="font-medium">
                    <Link
                      to="/routes/service"
                      search={{
                        selectedRouteId: selectedTrip.route_id,
                        selectedServiceId: selectedTrip.service_id,
                      }}
                      className="inline-flex items-center rounded px-1.5 py-0.5 hover:bg-primary/10 text-primary/80 transition-colors"
                    >
                      {selectedTrip.service_id}
              ...
              <div>
                <span className="text-xs text-muted-foreground">
                  {stopView === "stations" ? "Stations" : "Stops"}
                </span>
                <div
                  className={`font-medium ${stopTimes.length === 0 ? "text-muted-foreground/50" : ""}`}
                >
                  {stopTimesLoading ? "..." : stopTimes.length}
                </div>
              </div>
            </div>
          )}
          ...
          {(() => {
            ...
            const timelineDisabled = addingNoStops
              ? !hasAddTime
              : isEditing && !hasAnyTime && !hasAddTime
            const mapDisabled = addingNoStops ? !hasAddStop : false
            return (
              <Tabs
                value={tripView}
                onValueChange={(v) => {
                  const view = v as typeof tripView
                  setTripView(view)
                  updateSearch({ view: view === "timetable" ? undefined : view })
                }}
              >
                <TabsList className="h-8">
                  <TabsTrigger value="timetable" className="text-xs px-3 py-1">
                    Timetable
                  </TabsTrigger>
                  <TabsTrigger
                    value="timeline"
                  ...
                  <TabsTrigger value="map" className="text-xs px-3 py-1" disabled={mapDisabled}>
                    Map
                  </TabsTrigger>
                </TabsList>
              </Tabs>
          ...
          {stopTimesLoading ? (
            ...
            tripView === "timetable" ? (
              <>
                ...
                {editStops.length > 0 && (
                  <Timetable
                    editable
                    stops={editStops}
                    onUpdate={handleEditStopUpdate}
                    originalStops={stopTimes}
                    selectedStopIdx={selectedEditIdx}
                    onSelectStop={handleSelectEditStop}
                    addStopPreview={
                      selectedEditIdx == null && selectedAddStop
                        ? {
                            name: selectedAddStop.stop_name || selectedAddStop.stop_id,
                            stopId: selectedAddStop.stop_id,
                            arrival: addArrival,
                            departure: addDeparture,
                            stopSequence: addStopSequence,
                          }
              ...
              <>
                <Timeline
                  editable
                  trips={[{ trip: selectedTrip, stops: editStops }]}
                  onUpdateStop={handleTimelineStopUpdate}
                  onUpdateStopBoth={(_ti, si, arr, dep) =>
                    setEditStops((prev) =>
                      prev.map((s, i) =>
                        i === si ? { ...s, arrival_time: arr, departure_time: dep } : s,
                      ),
                    )
                  }
                  originalStops={stopTimes}
                  ...
                  onAddArrivalChange={setAddArrival}
                  onAddDepartureChange={setAddDeparture}
                  onInsertAt={(seq, arr, dep) => {
                    if (!arr && !dep) {
                      setAddStopSeqManual(null)
                      setAddStopId(undefined)
                      ...
                      setSelectedEditIdx(null)
                      setAddStopSeqManual(seq)
                      if (arr) setAddArrival(arr)
                      if (dep) setAddDeparture(dep)
                    }
                  }}
                  onDragStart={() => {
                    setUndoStack((prev) => [
                      ...prev.slice(-19),
                      {
                        stops: editStopsRef.current,
                        deleted: new Set(deletedStopIndicesRef.current),
                      },
                    ])
                  }}
                />
              ...
              <>
                ...
                <TripMap
                  ...
                  addStopPreview={
                    selectedEditIdx == null &&
                    selectedAddStop &&
                    Number.isFinite(Number(selectedAddStop.stop_lat)) &&
                    Number.isFinite(Number(selectedAddStop.stop_lon))
                      ? {
                          lon: Number(selectedAddStop.stop_lon),
                          lat: Number(selectedAddStop.stop_lat),
                          name: selectedAddStop.stop_name || selectedAddStop.stop_id,
                          stopSequence: addStopSequence,
                        }
                      : undefined
                  ...
                  onUpdateStopTime={(stopIdx, field, value) => {
                    pushEdit(
                      editStops.map((s, i) => (i === stopIdx ? { ...s, [field]: value } : s)),
                    )
                  }}
                  editPanel={
                    selectedEditIdx != null ||
            ...
            compareLoading && compareTrips.length > 0 ? (
              ...
              <TripMap
                trips={[
                  ...
                  ...compareTrips.map((ct) => ({
                    trip: ct,
                    stopTimes: compareStopTimesMap[ct.trip_id] || [],
                  })),
                ]}
                offsetPaths={compareOffsetPaths}
                selectedStopIdx={viewSelectedIdx}
                onSelectStop={setViewSelectedIdx}
                zoomToStopRef={zoomToStopRef}
                onClickAnyStop={(info) => {
                  setViewSelectedIdx(info.stopIdx)
                  setViewSelectedTripIdx(info.tripIdx)
                }}
              ...
              <>
                ...
                <div
                  onClick={(e) => {
                    ...
                    if (
                      !target.closest("[data-stop-idx]") &&
                      !target.closest("[data-stop-popup]")
                    ) {
                      setViewSelectedIdx(null)
                    }
                  }}
                >
                  <Timeline
                    ...
                    onSelectStopWithTrip={(ti, idx) => {
                      setViewSelectedTripIdx(ti)
                      setViewSelectedIdx(idx)
                    }}
                    hiddenTripIndices={hiddenTripIndices}
                  />
                  {viewSelectedIdx != null &&
                    (() => {
                      ...
                      const stopsAcrossTrips = allTrips
                        .map((t, ti) => {
                          const match =
                            t.stopTimes.find((s) => {
                              if (st.stop_id && s.stop_id && st.stop_id === s.stop_id) return true
                              if (
                                (st as any).parent_station &&
                                (s as any).parent_station &&
                                (st as any).parent_station === (s as any).parent_station
                              )
                                return true
                              return false
                      ...
                      return (
                        <div
                          data-stop-popup
                          className="rounded-md border bg-background p-3 text-sm space-y-2 mt-3"
                        >
                          <div className="flex items-center gap-2">
                            <span className="font-semibold">{st.stop_name || st.stop_id}</span>
                            <button
                              onClick={() => scrollToStop(viewSelectedIdx)}
                            ...
                            <button
                              onClick={() => setViewSelectedIdx(null)}
                          ...
                          <div className="flex items-center gap-2 text-xs text-muted-foreground">
                            ...
                            {(st as any).parent_station || st.location_type_name === "Station" ? (
                              <Link
                                to="/stations/info"
                                search={{
                                  selectedStationId: (st as any).parent_station || st.stop_id,
                                }}
                              ...
                              <Link
                                to="/stops/map"
                                search={{ selectedStopId: st.stop_id }}
                                className="inline-flex items-center gap-1 rounded px-1 py-0.5 hover:bg-primary/10 text-primary/80 transition-colors"
                              >
                                <BiMapPin className="h-3 w-3" />
                                Stop
                          ...
                          {hasCompare ? (
                            <div className="space-y-1 pt-1 border-t">
                              {stopsAcrossTrips.map(({ trip: t, st: s, ti }) => {
                                ...
                                const arrDiff =
                                  sArr != null && baseArr != null ? sArr - baseArr : null
                                const depDiff =
                                  sDep != null && baseDep != null ? sDep - baseDep : null
                                ...
                                return (
                                  <div
                                    key={ti}
                                    className={`flex items-center gap-2 text-xs rounded px-1.5 py-1 ${isPrimary ? "bg-primary/10 font-medium" : ""}`}
                                  >
                                    <span
                                      className="rounded-full shrink-0"
                                      style={{
                                        ...
                                        height: 8,
                                        backgroundColor:
                                          TRIP_LINE_COLORS[ti % TRIP_LINE_COLORS.length],
                                      }}
                                    />
                                    ...
                                    <span className="ml-auto text-muted-foreground whitespace-nowrap">
                                      {s.arrival_time || "\u2014"}
                                      {s.departure_time && s.departure_time !== s.arrival_time
                                        ? ` \u2192 ${s.departure_time}`
                                        : ""}
                                    </span>
                                    {!isPrimary && (arrDiff != null || depDiff != null) && (
                                      <span
                                        className={`text-[10px] font-medium ${(arrDiff ?? 0) > 0 ? "text-red-500" : (arrDiff ?? 0) < 0 ? "text-green-500" : "text-muted-foreground"}`}
                                      >
        ...
        <div className="space-y-3">
          ...
          >
            <TabsList className="h-8">
              ...
              <TabsTrigger value="map" className="text-xs px-3 py-1">
                Map
              </TabsTrigger>
            </TabsList>
          </Tabs>
          ...
          {compareLoading ? (
            ...
            >
              ...
              {viewSelectedIdx != null &&
                (() => {
                  const allTripsData = compareTrips.map((ct) => ({
                    trip: ct,
                    stopTimes: compareStopTimesMap[ct.trip_id] || [],
                  }))
                  ...
                  if (!st) return null
                  // Match stops across trips by stop_id or parent_station (same as map)
                  const stopsAcrossTrips = allTripsData
                    .map((t, ti) => {
                      const match =
                        t.stopTimes.find((s) => {
                          if (st.stop_id && s.stop_id && st.stop_id === s.stop_id) return true
                              ...
                              >
                                <span
                                  ...
                                  style={{
                                    width: 8,
                                    height: 8,
                                    backgroundColor: TRIP_LINE_COLORS[ti % TRIP_LINE_COLORS.length],
                                  }}
                                />
                                <span className="truncate max-w-[100px]">
                                  {t.trip_headsign || t.trip_id}
      ...
      {showStopForm && (
        <EntityForm
          ...
          setOpenValue={({ state }) => {
            if (!state) setShowStopForm(false)
          }}
          ClickInfo={undefined}
          setClickInfo={() => {}}
          type="stop"
