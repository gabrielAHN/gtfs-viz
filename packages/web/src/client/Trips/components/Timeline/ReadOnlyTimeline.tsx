...
export function TripTimeline({
  ...
  for (const { stops } of parsed) {
    ...
    for (const s of stops) {
      if (s.arrivalSec != null) {
        if (s.arrivalSec < minTime) minTime = s.arrivalSec
        if (s.arrivalSec > maxTime) maxTime = s.arrivalSec
      }
      if (s.departureSec != null) {
        if (s.departureSec < minTime) minTime = s.departureSec
        if (s.departureSec > maxTime) maxTime = s.departureSec
      }
    }
  }
  ...
  const timeToX = (sec: number) =>
    Math.round(paddingLeft + ((sec - minTime) / timeSpan) * chartWidth)
  ...
  for (
    let t = Math.floor(minTime / stepSeconds) * stepSeconds;
    t <= maxTime + stepSeconds;
    t += stepSeconds
  ) {
        ...
        <svg width={totalWidth} height={totalHeight} className="block" onMouseLeave={hideTooltip}>
          ...
          {parsed.map(({ trip, stops }, ti) => {
            ...
            const pts = stops
              ...
              .filter((p) => p.timeSec != null) as Array<{
              stopId: string
              name: string
              arrivalSec?: number
              departureSec?: number
              ...
              <g key={trip.trip_id}>
                ...
                {pts.map((p, pi) => {
                  ...
                  return (
                    <line
                      key={`vl-${pi}`}
                      x1={timeToX(pts[pi - 1].timeSec)}
                      y1={laneY}
                      x2={timeToX(p.timeSec)}
                      y2={laneY}
                      stroke={color}
                      strokeWidth={2}
                      opacity={0.3}
                ...
                {pts.map((p, pi) => {
                  ...
                  const isPrimarySelected =
                    (selectedTripIdx != null ? selectedTripIdx === ti : ti === 0) &&
                    selectedStopIdx === p.idx
                  ...
                  const editColor =
                    p.editStatus === "new" ? "#22c55e" : p.editStatus === "edit" ? "#f59e0b" : color
                  return (
                    <g
                      key={`d-${pi}`}
                      data-stop-idx={p.idx}
                    ...
                    >
                      {p.arrivalSec != null &&
                        p.departureSec != null &&
                        p.departureSec !== p.arrivalSec && (
                          <line
                            x1={timeToX(p.arrivalSec)}
                            y1={laneY}
                            x2={timeToX(p.departureSec)}
                            y2={laneY}
                            stroke={isStopHighlighted ? "#f59e0b" : editColor}
                            strokeWidth={6}
                            opacity={isStopHighlighted ? 0.4 : 0.25}
                            strokeLinecap="round"
                          />
                        )}
                      <circle
                        cx={x}
                        cy={laneY}
                        r={isStopHighlighted ? dotR + 2 : dotR}
                        fill={isPrimarySelected ? "#f59e0b" : isStopHighlighted ? color : editColor}
                        stroke={isStopHighlighted ? "#f59e0b" : "var(--background)"}
                        strokeWidth={isStopHighlighted ? 2.5 : 2}
                      />
                    </g>
                  )
                })}
                ...
                {pts.map((p, pi) => {
                  ...
                  const depStr =
                    p.departureSec != null && p.departureSec !== p.arrivalSec
                      ? fmtTime(p.departureSec)
                      : ""
                    ...
                    <circle
                      ...
                      onMouseMove={(e) =>
                        showTooltip(e, [
                          ...
                          timeLine,
                          ...(p.editStatus
                            ? [p.editStatus === "new" ? "Added stop" : "Edited stop"]
                            : []),
                        ])
                      }
                      onMouseLeave={hideTooltip}
