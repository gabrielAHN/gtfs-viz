...
export function EditableTimetable({
  ...
  const handleDrop = (e: React.DragEvent, dropIdx: number) => {
    ...
    if (selectedStopIdx != null) {
      ...
      else if (fromIdx < selectedStopIdx && dropIdx >= selectedStopIdx)
        onSelectStop?.(selectedStopIdx - 1)
      else if (fromIdx > selectedStopIdx && dropIdx <= selectedStopIdx)
        onSelectStop?.(selectedStopIdx + 1)
    }
            ...
            {(() => {
              ...
              if (originalStops) {
                for (let i = 0; i < stops.length; i++) {
                  ...
                  const orig =
                    origIdx != null && origIdx < originalStops.length
              ...
              const handleInsertClick = (pos: number) => {
                ...
                const prevSec =
                  timeToSec(prevStop?.departure_time) ?? timeToSec(prevStop?.arrival_time)
                const nextSec =
                  timeToSec(nextStop?.arrival_time) ?? timeToSec(nextStop?.departure_time)
                let time = ""
              ...
              const previewRow = (seq: number) => (
                <tr
                  ...
                  className="border-t border-dashed border-amber-400 bg-amber-50 dark:bg-amber-950/20 cursor-pointer hover:bg-amber-100 dark:hover:bg-amber-950/40 transition-colors"
                  onClick={() => onInsertAt?.(seq, "", "")}
                >
                  <td className="px-1 py-2 text-amber-500">
                    <BiX className="h-4 w-4" />
                  </td>
                  <td className="px-3 py-2 text-amber-600 font-medium">{seq}</td>
                  <td className="px-3 py-2 font-medium text-amber-700 dark:text-amber-400">
                    {addStopPreview!.name || (
                      <span className="italic text-amber-500/60">Select stop...</span>
                    )}
                  </td>
                  <td className="px-3 py-2 text-amber-600/70 text-xs">
                    {addStopPreview!.stopId || "\u2014"}
                  </td>
                  <td className="px-3 py-2 text-amber-600">
                    {addStopPreview!.arrival || "\u2014"}
                  </td>
                  <td className="px-3 py-2 text-amber-600">
                    {addStopPreview!.departure || "\u2014"}
                  </td>
                ...
                if (i < stops.length) {
                  ...
                  if (
                    ...
                    const prevWasVisible =
                      i === 0 || stopStatus.has(i - 1) || selectedStopIdx === i - 1
                    if (prevWasVisible) {
                      rows.push(
                    ...
                    >
                      ...
                      <td className="px-3 py-2 text-muted-foreground">
                        <div className="flex items-center gap-1">
                          {stopStatus.has(i) && (
                            <EditIndicator status={stopStatus.get(i)} className="h-3.5 w-3.5" />
                          )}
                          <span>{seq}</span>
                        </div>
                      </td>
                      ...
                      <td className="px-1 py-2">
                        <button
                          onClick={(e) => {
                            ...
                            if (selectedStopIdx === i) onSelectStop?.(null)
                          }}
