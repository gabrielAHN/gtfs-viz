...
export function TripStopSequence({
  stopTimes,
  view = "stops",
}: {
  ...
  if (view === "stations") {
    const grouped: Array<{
      ...
      edit_status?: TripStopTime["edit_status"]
    }> = []
    for (const st of stopTimes) {
      const hasParent =
        st.parent_station && st.parent_station !== "" && st.parent_station !== st.stop_id
      ...
      if (last && last.station_name === name) {
        ...
        grouped.push({
          station_name: name,
          station_id: sid,
          isStation: !!hasParent || isStationType,
          sequence: grouped.length + 1,
          stop_count: 1,
          arrival_time: st.arrival_time || "",
          departure_time: st.departure_time || "",
          edit_status: st.edit_status,
        })
    }
    return (
              ...
              {grouped.length > 0 ? (
                grouped.map((g, i) => (
                  <tr key={i} className="border-t hover:bg-muted/50">
                    ...
                    <td className="px-3 py-2 font-medium">
                      {g.station_id ? (
                        g.isStation ? (
                          <Link
                            to="/stations/info"
                            search={{ selectedStationId: g.station_id }}
                          ...
                          <Link
                            to="/stops/map"
                            search={{ selectedStopId: g.station_id }}
                            onClick={(e) => e.stopPropagation()}
                            className="inline-flex items-center rounded px-1.5 py-0.5 text-sm hover:bg-primary/10 text-primary/80 transition-colors"
                          >
                            {g.station_name}
                    ...
                    <td className="px-3 py-2">{g.arrival_time || "\u2014"}</td>
                    <td className="px-3 py-2">{g.departure_time || "\u2014"}</td>
                  </tr>
                ))
              ) : (
                <tr>
          ...
          <tbody>
            {stopTimes.length > 0 ? (
              stopTimes.map((st, i) => {
                ...
                const linkClass =
                  "inline-flex items-center rounded px-1.5 py-0.5 hover:bg-primary/10 text-primary/80 transition-colors"
                return (
                  <tr key={i} className="border-t hover:bg-muted/50">
                    <td className="px-3 py-2 text-muted-foreground">
                      <span className="flex items-center gap-1">
                      ...
                      {st.stop_id ? (
                        hasParent || st.location_type_name === "Station" ? (
                          <Link
                            to="/stations/info"
                            search={{
                              selectedStationId: hasParent ? st.parent_station! : st.stop_id,
                            }}
                            onClick={(e) => e.stopPropagation()}
                    ...
                    <td className="px-3 py-2">{st.arrival_time || "\u2014"}</td>
                    <td className="px-3 py-2">{st.departure_time || "\u2014"}</td>
                    <td className="px-3 py-2 text-muted-foreground text-xs">
                      {stationId ? (
                        <Link
                          to="/stations/info"
                          search={{ selectedStationId: stationId }}
                          onClick={(e) => e.stopPropagation()}
                          className={`${linkClass} text-xs`}
                        >
                          {st.station_name || stationId}
                        </Link>
                      ) : (
                        "\u2014"
                      )}
  ...
  return (
    <div>
      <div className="mb-2 flex items-center gap-2 flex-wrap">
        ...
        {(trip as any).route_name && (
          <span className="text-[10px] text-muted-foreground/70 truncate">
            {(trip as any).route_name}
          </span>
        )}
        {(trip as any).service_id && (
