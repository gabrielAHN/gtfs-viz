...
export function MapSection({
  ...
  const layers = useMemo(() => {
    ...
    const bandedTrips = new Set<number>()
    {
      ...
      const GAP_PX = 7
      const bands = visiblePaths.map((p) => (p as any).band).filter((b) => b?.lons && b?.lats)
      const fanBundleRef = fanBundleRefFor(largestBundleIn(bands))
      for (const p of visiblePaths) {
        const band = (p as any).band
        if (!band?.lons || !band?.lats) continue
        const [built] = buildRouteShapePaths([{ ...band, route_id: String(p.tripIdx) }], {
          cleaned: true,
          getGapPx: () => GAP_PX,
        })
        if (!built) continue
        bandedTrips.add(p.tripIdx)
        const flat = built.path
        const fanCodes = built.fanCodes
        const color = safeHexToRgb(TRIP_LINE_COLORS[p.tripIdx % TRIP_LINE_COLORS.length])
        layerList.push(
          new FannedPathLayer({
            id: `trip-path-band-${p.tripIdx}`,
            data: [{ path: flat, fanCodes, tripIdx: p.tripIdx }],
            getPath: (d: any) => d.path,
            getFanCodes: (d: any) => d.fanCodes,
