...
import FannedPathLayer, {
  encodeFanZ,
} from "@/client/Routes/AllRoutes/RoutesMap/Components/FannedPathLayer"
import {
  fanRampForZoom,
  fanMaxPxForZoom,
} from "@/client/Routes/AllRoutes/RoutesMap/Components/MapSection"
...
interface MapSectionProps {
  ...
  segments: Segment[]
  paths: Array<{ path?: [number, number][]; band?: any; tripIdx: number }>
...
export function MapSection({
  ...
  const handleClick = useCallback(
    (event: any) => {
      ...
      if (
        event?.object &&
        (event.layer?.id === "trip-stops" ||
          event.layer?.id === "trip-stops-overlap" ||
          event.layer?.id === "trip-stops-overlap-ring")
      ) {
        let stop: StopPoint
        if (event.layer?.id === "trip-stops" || event.layer?.id === "trip-stops-overlap") {
          ...
          stop = obj.pt
            ? (obj.pt as StopPoint)
            : stopPointsRef.current.find(
                (p) =>
                  `${p.lon.toFixed(3)},${p.lat.toFixed(3)}` ===
                  `${Number(obj.lon).toFixed(3)},${Number(obj.lat).toFixed(3)}`,
              ) || obj
        }
        // Toggle: deselect if same stop clicked again
        if (
          clickedStop &&
          clickedStop.stopIdx === stop.stopIdx &&
          clickedStop.tripIdx === stop.tripIdx
        ) {
          onClickStop(null)
          onSelectStop?.(null)
        } else {
      ...
      if (
        event?.object &&
        (event.layer?.id === "trip-segments" || event.layer?.id?.startsWith("trip-path"))
      ) {
        if (event.layer?.id === "trip-segments") {
          const seg = event.object as Segment
          const isSame =
            clickedSegment &&
            clickedSegment.fromIdx === seg.fromIdx &&
            clickedSegment.toIdx === seg.toIdx &&
            clickedSegment.tripIdx === seg.tripIdx
          ...
          if (pathCoords && pathCoords.length === 2) {
            ...
            const findStop = (coord: [number, number]) => {
              ...
              let bd = Infinity
              for (const p of pts) {
                const d = (p.lon - coord[0]) ** 2 + (p.lat - coord[1]) ** 2
                if (d < bd) {
                  bd = d
                  best = p
                }
            ...
            if (fromPt && toPt) {
              bestSeg = {
                from: pathCoords[0],
                to: pathCoords[1],
                fromIdx: fromPt.stopIdx,
                toIdx: toPt.stopIdx,
                fromName: fromPt.name,
                toName: toPt.name,
                fromTime: fromPt.departureTime || fromPt.arrivalTime,
                toTime: toPt.arrivalTime || toPt.departureTime,
                tripIdx,
                disabled: false,
                fromStopId: fromPt.stopId,
                toStopId: toPt.stopId,
                fromStation: fromPt.parentStation,
                toStation: toPt.parentStation,
              }
            }
          }
          if (!bestSeg) {
            ...
            for (let i = 1; i < pts.length; i++) {
              ...
              if (d < bestDist) {
                ...
                bestSeg = {
                  ...
                  fromName: a.name,
                  toName: b.name,
                  fromTime: a.departureTime || a.arrivalTime,
                  toTime: b.arrivalTime || b.departureTime,
                  tripIdx,
                  disabled: false,
                  fromStopId: a.stopId,
                  toStopId: b.stopId,
                  fromStation: a.parentStation,
                  toStation: b.parentStation,
                }
              }
            }
          }
          if (bestSeg) {
            const isSame =
              clickedSegment &&
              clickedSegment.fromIdx === bestSeg.fromIdx &&
              clickedSegment.toIdx === bestSeg.toIdx &&
              clickedSegment.tripIdx === bestSeg.tripIdx
  ...
  const layers = useMemo(() => {
    ...
    if (isCompare) {
      ...
      for (const [locKey, pts] of byKey) {
        ...
        const isStationGroup =
          pts.some((p) => p.parentStation && p.parentStation.length > 0) &&
        ...
        if (isStationGroup) {
          ...
          for (const pt of pts) {
            if (!perTrip.has(pt.tripIdx)) perTrip.set(pt.tripIdx, pt)
          }
        ...
        for (const pt of filteredPts) {
          const next = visibleStops.find(
            (p) => p.tripIdx === pt.tripIdx && p.stopIdx === pt.stopIdx + 1,
          )
          if (next) {
    ...
    const getStopCoord = (pt: StopPoint): [number, number] =>
      stopCoordMap.get(`${pt.tripIdx}-${pt.stopIdx}`) || [pt.lon, pt.lat]
    ...
    if (isCompare) {
      ...
      for (const { pt, next } of rawSegs) {
        ...
        segRenders.push({
          ...
          tripIdx: pt.tripIdx,
          bothOverlap: shared,
          fromStopId: pt.stopId,
          toStopId: next.stopId,
          fromStation: pt.parentStation,
          toStation: next.parentStation,
        })
      }
    }
    ...
    if (clickedSegment) {
      ...
      const matchingRendered = segRenders.filter((s) => {
        ...
        return (
          (sFromK === clickedFromK && sToK === clickedToK) ||
    ...
    const bandedTrips = new Set<number>()
    {
      const fanRamp = fanRampForZoom(curZoom)
      const fanMaxPx = fanMaxPxForZoom(curZoom)
      const GAP_PX = 7
      for (const p of visiblePaths) {
        const band = (p as any).band
        if (!band?.lons || !band?.lats) continue
        const n = Math.min(band.lons.length, band.lats.length)
        if (n < 2) continue
        bandedTrips.add(p.tripIdx)
        const flat = new Float64Array(n * 3)
        for (let i = 0; i < n; i++) {
          flat[i * 3] = band.lons[i]
          flat[i * 3 + 1] = band.lats[i]
          flat[i * 3 + 2] = encodeFanZ(
            (band.slots ? band.slots[i] : 0) * GAP_PX,
            band.turnRadii ? band.turnRadii[i] : 5000,
            band.bandCounts ? band.bandCounts[i] : 1,
            GAP_PX,
          )
        }
        ...
        layerList.push(
          new FannedPathLayer({
            id: `trip-path-band-${p.tripIdx}`,
            data: [{ path: flat, tripIdx: p.tripIdx }],
            getPath: (d: any) => d.path,
    ...
    if (isCompare) {
      const overlapSegs = segRenders.filter((s) => s.bothOverlap && !bandedTrips.has(s.tripIdx))
      if (overlapSegs.length > 0) {
        ...
        for (const ti of visibleTripIds) {
          ...
          const pathData = tripSegs.map((s) => {
            ...
            const sharingTrips = overlapSegs
              .filter((o) => segStopKey(o) === key)
              .map((o) => o.tripIdx)
      ...
      const soloSegs = segRenders.filter((s) => !s.bothOverlap && !bandedTrips.has(s.tripIdx))
      ...
      visiblePaths
        .filter((p) => !bandedTrips.has(p.tripIdx) && p.path)
        .forEach((p) => {
          const color = safeHexToRgb(TRIP_LINE_COLORS[p.tripIdx % TRIP_LINE_COLORS.length])
          layerList.push(
            new PathLayer({
              id: `trip-path-${p.tripIdx}`,
              data: [p],
              getPath: (d: any) => d.path,
              getColor: [...color, 230] as [number, number, number, number],
              getWidth: 4,
        ...
        layerList.push(
          new LineLayer({
            ...
            getColor: (d: Segment) => {
              ...
              const isClicked =
                clickedSegment?.fromIdx === d.fromIdx && clickedSegment?.toIdx === d.toIdx
              const rgb = safeHexToRgb(TRIP_LINE_COLORS[0])
              return isClicked
                ? ([...rgb, 255] as [number, number, number, number])
                : ([...rgb, 200] as [number, number, number, number])
            },
            getWidth: isCompare ? 12 : 10,
            widthUnits: "pixels",
            pickable: true,
            updateTriggers: {
              getColor: [
                deletedStopIndices.size,
                clickedSegment?.fromIdx,
                clickedSegment?.toIdx,
                isCompare,
                highlightedSegmentRange?.fromStopIdx,
                highlightedSegmentRange?.toStopIdx,
    ...
    const overlapStops = isCompare
      ? visibleStops
          .filter((d) => overlapInfo.has(`${d.tripIdx}-${d.stopIdx}`))
          .map((d) => {
            const c = stopCoordMap.get(`${d.tripIdx}-${d.stopIdx}`)
            return c ? { ...d, lon: c[0], lat: c[1] } : d
          })
      : []
    const soloStops = isCompare
      ? visibleStops.filter((d) => !overlapInfo.has(`${d.tripIdx}-${d.stopIdx}`))
      : visibleStops
    ...
    if (overlapStops.length > 0 && isCompare) {
      ...
      const dotR = (count: number) =>
        count <= 3 ? baseR : Math.max(3, Math.round((baseR * 3) / count))
      const dotSp = (count: number) => dotR(count) * 2
      const capsuleW = (count: number) => {
        const r = dotR(count)
        return (count - 1) * r * 2 + r * 2
      }
      layerList.push(
        new IconLayer({
          id: "trip-stops-overlap-ring",
          data: hitData,
          ...
          getSize: (d: { count: number }) =>
            Math.round(capsuleW(d.count) * (d.count <= 2 ? 0.7 : 0.5)),
    ...
    layerList.push(
      new TextLayer({
        id: "trip-stop-labels",
        data: visibleStops.filter((d) => !d.disabled),
        getPosition: (d: StopPoint) => {
          const c = stopCoordMap.get(`${d.tripIdx}-${d.stopIdx}`)
          return c || [d.lon, d.lat]
        },
        getText: (d: StopPoint) => {
          if (previewSeqNum && d.tripIdx === 0 && d.sequence >= previewSeqNum)
            return String(d.sequence + 1)
        ...
        sizeUnits: "pixels",
        updateTriggers: {
          getText: [previewSeqNum],
          getColor: [isDark],
          getPixelOffset: [overlapInfo.size, segR],
          getSize: [overlapInfo.size, segR],
        },
      }),
    )
    ...
    if (activePreview) {
      ...
      layerList.push(
        new TextLayer({
          id: "preview-stop-label",
          data: [{ ...activePreview, _label: previewLabel }],
          getPosition: (d: any) => [d.lon, d.lat],
          getText: (d: any) => d._label,
          getSize: 11,
      ...
      if (seq && stopPoints.length > 0) {
        ...
        if (prevStop)
          connectionData.push({
            from: [prevStop.lon, prevStop.lat],
            to: [activePreview.lon, activePreview.lat],
          })
        if (nextStop)
          connectionData.push({
            from: [activePreview.lon, activePreview.lat],
            to: [nextStop.lon, nextStop.lat],
          })
        ...
        layerList.push(
          new LineLayer({
            ...
            data: [
              {
                from: [clickedStop.lon, clickedStop.lat],
                to: [activePreview.lon, activePreview.lat],
              },
            ],
            getSourcePosition: (d: any) => d.from,
            getTargetPosition: (d: any) => d.to,
            getColor: [34, 197, 94, 150],
            getWidth: 3,
            widthUnits: "pixels",
            pickable: false,
          }),
