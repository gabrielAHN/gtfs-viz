...
import { queryWithSpatial } from "@/lib/cli/nativeDuckDb"
...
const tableExists = async (conn: any, tableName: string): Promise<boolean> => {
  const rows = await executeQuery(
    conn,
    `SELECT COUNT(*) AS n FROM information_schema.tables WHERE lower(table_name) = lower('${escapeSql(tableName)}')`,
  )
  return Number(rows[0]?.n ?? 0) > 0
}

const loadSpatial = async (conn: any): Promise<void> => {
  if (conn?.__gtfsVizCliNative) return
  try {
...
export const bandedRouteIds = async (conn: any): Promise<Set<string>> => {
  try {
    if (await tableExists(conn, "RouteShapeMacroVersion")) {
      ...
      if (!(await tableExists(conn, "RouteShapeBandsTable"))) {
        await resetCleanupTables(conn)
        return new Set()
      }
      const rows = await executeQuery(
...
export const prepareRouteShapeLanes = async (
  ...
  try {
    ...
    for (let i = 0; i < groups.length; i++) {
      await queryWithSpatial(conn, `INSERT INTO RouteShapeLanesTable SELECT * FROM ${groups[i]}()`)
...
export type ShapePathRecord = {
  ...
  turnRadii: Float32Array | null
  bandCounts: Float32Array | null
}
...
const toFloat32 = (v: any): Float32Array => {
  if (v instanceof Float32Array) return v
  if (v && typeof v.toArray === "function") {
    const a = v.toArray()
    return a instanceof Float32Array ? a : Float32Array.from(a, Number)
  }
  return Float32Array.from(Array.isArray(v) ? v : [], Number)
}
  ...
  const read = async (sql: string, cleaned: boolean): Promise<ShapePathRecord[]> => {
    ...
    for (let i = 0; i < n; i++) {
      ...
      out.push({
        ...
        lons,
        lats,
        slots: cleaned ? toFloat32(cSlots?.get(i)) : null,
        turnRadii: cleaned ? toFloat32(cTurn?.get(i)) : null,
        bandCounts: cleaned ? toFloat32(cCounts?.get(i)) : null,
      })
    }
    return out
  }
...
export const shapeRowsToPaths = (rows: any[]): ShapePathRecord[] => {
  ...
  for (const { rec, pts } of groups.values()) {
    ...
    const slots = cleaned ? new Float32Array(n) : null
    const turnRadii = cleaned ? new Float32Array(n) : null
    const bandCounts = cleaned ? new Float32Array(n) : null
    for (let i = 0; i < n; i++) {
      lons[i] = pts[i].lon
      lats[i] = pts[i].lat
