...
const Datafiles = [
  ...
  },
  {
    orgTable: {
      name: "pathways",
      file: "pathways.csv",
      removeList: ["row_id", "pathway_mode_name", "direction_type"],
    },
    editTable: {
...
const isCliNativeConn = (conn: any): boolean => Boolean(conn?.__gtfsVizCliNative)
...
export const exportingData = async ({ conn, FileTypes }) => {
  ...
  try {
    const filteredDatafiles = Datafiles.filter((fileInfo) => FileTypes[fileInfo.orgTable.name])
    ...
    for (const fileInfo of filteredDatafiles) {
      ...
      if (isCliNativeConn(conn)) {
        ...
        csvContent = await conn._bindings.copyFileToBuffer(fileInfo.orgTable.file)
...
const CreateExportQuery = async (conn, fileInfo) => {
  ...
  const editColumnList = await generateDynamicSelectQuery(conn, fileInfo.editTable.name)

  const mappedColumns = orgColumnList.map((col) =>
    editColumnList.includes(col) ? `COALESCE(edt.${col}, NULL) AS ${col}` : `NULL AS ${col}`,
  )
