...
CREATE OR REPLACE MACRO prepare_route_shape_lanes(
  ...
  WITH cand AS (
    SELECT CASE WHEN p_kind = 'trip' THEN t.trip_id ELSE t.route_id END AS route_id,
           t.shape_id, COUNT(*) AS tn, MIN(r.route_type) AS route_type
    FROM TripsView t
    LEFT JOIN RoutesView r ON r.route_id = t.route_id
  ...
  simplified AS (
    ...
    WHERE s.shape_pt_lat IS NOT NULL AND s.shape_pt_lon IS NOT NULL
    GROUP BY r.route_id, r.shape_id, r.route_type
    HAVING COUNT(*) >= 2
  ),
  exploded AS (
    SELECT route_id, shape_id, route_type,
           UNNEST(ST_Dump(ST_Points(geom))) AS d
    FROM simplified
  ),
  clean AS (
    SELECT route_id, route_type, shape_id,
           d.path[1] AS shape_pt_sequence,
  ...
  dense AS (
    ...
    FROM (
      ...
      FROM seg WHERE nlat IS NOT NULL
    ) d
    JOIN (SELECT UNNEST(range(0, 120)) AS j) g ON g.j < d.k
    UNION ALL
    SELECT route_id, route_type, shape_id, shape_pt_sequence, lat, lon
    FROM seg WHERE nlat IS NULL
  ),
  neigh AS (
  ...
  vtx AS MATERIALIZED (
    SELECT DENSE_RANK() OVER (ORDER BY route_id, shape_id) AS rid,
           route_id, shape_id, shape_pt_sequence, lat, lon, coslat,
           ...
           CAST(FLOOR(lat / (reach_meters / 111320.0)) AS BIGINT) * 4194304
             + CAST(FLOOR((lon * coslat) / (reach_meters / 111320.0)) AS BIGINT) AS cell
    FROM dir
  ),
  nbr AS (
    SELECT a.rid, a.vidx, b.rid AS nrid, b.vidx AS nvidx,
    ...
    FROM (SELECT v.rid, v.vidx, v.lat, v.lon, v.coslat, v.ux, v.uy,
                 v.cell + o.dy * 4194304 + o.dx AS pcell
          FROM vtx v
          CROSS JOIN (SELECT UNNEST([-1, 0, 1]) AS dy) oy
          CROSS JOIN (SELECT UNNEST([-1, 0, 1]) AS dx) ox
          CROSS JOIN LATERAL (SELECT ox.dx AS dx, oy.dy AS dy) o) a
    JOIN vtx b
      ON b.cell = a.pcell
     AND b.rid <> a.rid
     AND ABS(a.ux * b.ux + a.uy * b.uy) >= 0.82
     AND ABS(-((b.lon - a.lon) * a.coslat * 111320.0) * a.uy + ((b.lat - a.lat) * 111320.0) * a.ux) <= reach_meters
     AND ABS(((b.lon - a.lon) * a.coslat * 111320.0) * a.ux + ((b.lat - a.lat) * 111320.0) * a.uy) <= 50.0
  ),
  strands AS (
    SELECT rid, vidx, nrid,
           arg_min(nvidx, ABS(along)) AS nvidx,
           arg_min(perp, ABS(along)) AS perp,
           arg_min(hdg, ABS(along)) AS hdg,
           arg_min(cx, ABS(along)) AS cx
    FROM nbr
    GROUP BY rid, vidx, nrid
  ),
  strands_s AS MATERIALIZED (
    SELECT rid, vidx, nrid, nvidx, perp, hdg, cx,
           AVG(perp) OVER (PARTITION BY rid, nrid ORDER BY vidx
                           ROWS BETWEEN 15 PRECEDING AND 15 FOLLOWING) AS perp_s,
           vidx - ROW_NUMBER() OVER (PARTITION BY rid, nrid ORDER BY vidx) AS run_id
    FROM strands
  ),
  pair_med AS (
    SELECT rid, nrid, run_id, MEDIAN(perp_s) AS med_perp
    FROM strands_s
    GROUP BY rid, nrid, run_id
  ),
  all_strands AS MATERIALIZED (
    SELECT rid, vidx, rid AS nrid, vidx AS nvidx, 0.0 AS perp, 0.0 AS perp_s, 1.0 AS hdg,
           CASE WHEN -uy + 0.37 * ux > 0 THEN 1.0 ELSE -1.0 END AS cx,
           CAST(NULL AS BIGINT) AS run_id
    FROM vtx
    UNION ALL
    SELECT rid, vidx, nrid, nvidx, perp, perp_s, hdg, cx, run_id FROM strands_s
  ),
  anchor AS (
    SELECT rid, vidx,
           arg_min(CASE WHEN hdg >= 0 THEN 1.0 ELSE -1.0 END, nrid) AS sgn
    FROM all_strands
    GROUP BY rid, vidx
  ),
  bundle0 AS MATERIALIZED (
    SELECT s.rid, s.vidx,
           ...
           CAST(COUNT(*) FILTER (WHERE s.nrid <> s.rid AND
    ...
    FROM all_strands s
    JOIN anchor k USING (rid, vidx)
    LEFT JOIN pair_med p
      ON p.rid = s.rid AND p.nrid = s.nrid AND p.run_id = s.run_id
    GROUP BY s.rid, s.vidx
  ),
  ranked AS (
    SELECT s.rid, s.vidx,
           CAST(COUNT(*) FILTER (WHERE
             ...
             OR ((CASE WHEN n.sgn * (CASE WHEN s.hdg >= 0 THEN 1.0 ELSE -1.0 END) = r.sgn
                        THEN n.band_index ELSE n.band_count - 1 - n.band_index END) = r.band_index
                 AND s.nrid < s.rid)) AS INTEGER) AS lane
    FROM all_strands s
    JOIN bundle0 r ON r.rid = s.rid AND r.vidx = s.vidx
    JOIN bundle0 n ON n.rid = s.nrid AND n.vidx = s.nvidx
    WHERE s.nrid <> s.rid
    GROUP BY s.rid, s.vidx
  ),
  bundle AS (
    SELECT b.rid, b.vidx, b.band_count,
           COALESCE(k.lane, 0) AS band_index, b.sgn, b.shift_raw
    FROM bundle0 b
    LEFT JOIN ranked k USING (rid, vidx)
  ),
  laterals AS (
    ...
    FROM vtx v
    JOIN bundle b USING (rid, vidx)
    WINDOW w AS (PARTITION BY v.rid ORDER BY v.vidx),
           wsm AS (PARTITION BY v.rid ORDER BY v.vidx
...
CREATE OR REPLACE MACRO finish_route_shape_bands(p_route_ids) AS TABLE (
  ...
  fin1 AS (
    SELECT o.route_id, o.shape_id, o.ord, o.x, o.y,
  ...
  fin1d AS MATERIALIZED (
    SELECT route_id, shape_id, ord, x, y,
  ...
  ck1 AS MATERIALIZED (
    SELECT route_id, shape_id, band_index, band_count, slot_s, turn_rw, x, y,
           ROW_NUMBER() OVER w AS ord,
  ...
  SELECT sm.route_id, rv.route_name, rv.route_color_hex, rv.route_text_color_hex, sm.shape_id,
         ...
         CAST(sm.band_index AS SMALLINT) AS band_index, CAST(sm.band_count AS SMALLINT) AS band_count,
         CAST(sm.slot_s AS FLOAT) AS slot, CAST(sm.turn_rw AS FLOAT) AS turn_radius
  FROM smoothed sm
