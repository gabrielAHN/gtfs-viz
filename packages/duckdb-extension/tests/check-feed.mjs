...
WITH expected AS (
  ...
  SELECT shape_id,
    first(shape_pt_lat ORDER BY shape_pt_sequence) oa, first(shape_pt_lon ORDER BY shape_pt_sequence) ob,
    last(shape_pt_lat ORDER BY shape_pt_sequence) oc, last(shape_pt_lon ORDER BY shape_pt_sequence) od
  FROM shapes WHERE shape_id IN (SELECT DISTINCT shape_id FROM RouteShapeBandsTable) GROUP BY shape_id
), ends AS (
  SELECT b.route_id, b.shape_id,
    first(b.shape_pt_lat ORDER BY b.shape_pt_sequence) a, s.oa,
    first(b.shape_pt_lon ORDER BY b.shape_pt_sequence) b, s.ob,
    last(b.shape_pt_lat ORDER BY b.shape_pt_sequence) c, s.oc,
    last(b.shape_pt_lon ORDER BY b.shape_pt_sequence) d, s.od
  FROM RouteShapeBandsTable b JOIN src_ends s USING (shape_id) GROUP BY b.route_id, b.shape_id, s.oa, s.ob, s.oc, s.od
)
SELECT (SELECT count(*) FROM expected) eligible_routes,
       ...
       (SELECT count(*) FROM (SELECT route_id, shape_pt_sequence, slot_s, band_count,
                                     LAG(slot_s) OVER (PARTITION BY route_id, shape_id ORDER BY shape_pt_sequence) p,
                                     LAG(band_count) OVER (PARTITION BY route_id, shape_id ORDER BY shape_pt_sequence) pc
                              FROM RouteShapeLanesTable)
        WHERE band_count = pc AND abs(slot_s - p) > 0.02) lane_steps_inside_bundles,
       (SELECT count(*) FROM RouteShapeLanesTable) lane_vertices,
       (SELECT count(*) FILTER (WHERE abs(slot - p) > 0.5) FROM (SELECT slot, LAG(slot) OVER (PARTITION BY route_id, shape_id ORDER BY shape_pt_sequence) p FROM RouteShapeBandsTable)) big_slot_jumps,
       (SELECT round(max(abs(slot - p)), 3) FROM (SELECT slot, LAG(slot) OVER (PARTITION BY route_id, shape_id ORDER BY shape_pt_sequence) p FROM RouteShapeBandsTable)) max_slot_jump
FROM RouteShapeBandsTable;
`)[0];
report.lane_step_ratio = +(report.lane_steps_inside_bundles / report.lane_vertices).toFixed(4);
