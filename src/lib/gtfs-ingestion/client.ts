...
export async function runIngestion(
  ...
  try {
    ...
    if (!skipReformat) {
      ...
      await conn.query(`DROP TABLE stops_temp`);
      ...
      if (hasPathwaysData) {
        ...
        try {
          ...
          await conn.query(`DROP TABLE pathways_temp`);
        } catch (reformatError) {
          ...
          try {
            ...
            } else if (tableNames.includes('pathways_temp')) {
              
              await conn.query(`DROP TABLE pathways_temp`);
            }
          } catch (cleanupError) {
            logger.error('Error during pathways cleanup:', cleanupError);
          }
        }
      } else {
